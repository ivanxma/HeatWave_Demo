#!/usr/bin/env python3
"""
Reusable starting point for an Admin auto-update worker.

Adapt:
- heatwave-demo
- status/log file locations supplied by the Flask app
- service names if the app does not use <app-slug>-http.service and <app-slug>-https.service
"""
...
class UpdateWorker:
    ...
    def detect_deploy_mode_and_services(self, runtime_env):
        ...
        if env_mode in {"http", "https", "both", "none"}:
            ...
            if env_mode == "both":
                return env_mode, [service for service in (HTTP_SERVICE, HTTPS_SERVICE) if shutil.which("systemctl")]
            return env_mode, []
    ...
    def schedule_service_restart(self, service_names, completion_message):
        ...
        if shutil.which("systemd-run"):
            transient_unit_name = f"heatwave-demo-self-update-{os.getpid()}"
    ...
    def run(self):
        ...
        if sudo_ready:
            ...
            self.log_step(
                "Running setup",
                "Passwordless sudo is unavailable from the running service. Refreshing the repository and Python environment in unprivileged mode.",
            )
            if sudo_error:
                self.append_log(f"Passwordless sudo check failed: {sudo_error}")
            self.append_log(
                "setup.sh will skip privileged steps such as package installation, firewall changes, and systemd unit rewrites."
            )
